...
// =====================
mongoose.connect(
  process.env.MONGO_URI || "mongodb+srv://admin:admin3467@cluster0.sg5qkck.mongodb.net/demo-game",
  {
    autoIndex: true,
  }
);
...
app.post("/admin/update-player", async (req, res) => {
  ...
  if (!player) player = await Player.create({ id, name, balance });
  else {
    player.name = name;
